...
class MySQLVisualizer:
    def __init__(self):
        ...
        try:
            ...
            self.connection_params = {
                'host': 'localhost',
                'user': 'root',
                'password': 'Christevy',  # Votre mot de passe
                'database': 'projet_sql_py'  # Votre base de données
            }
        ...
        # Création des dossiers pour l'export et les visualisations
        
        self.visualizations_dir = "visualization"
        
        os.makedirs(self.export_dir, exist_ok=True)
        os.makedirs(self.visualizations_dir, exist_ok=True)
        
        
        print(f"📁 Dossier visualizations : {os.path.abspath(self.visualizations_dir)}")
    ...
    def safe_plot(self, fig, filename):
        """Sauvegarde sécurisée d'un graphique dans le dossier visualization"""
        try:
            plt.tight_layout()
            # Sauvegarde dans le dossier visualization
    ...
    def export_dataframe(self, df, filename):
        """Exporte un DataFrame en CSV dans le dossier export"""
        try:
            if not df.empty:
                ...
                print(f"⚠️  DataFrame vide, aucun export pour {filename}")
        except Exception as e:
            print(f"❌ Erreur lors de l'export du CSV {filename}: {e}")
    ...
    def evolution_ventes_temps(self):
        ...
        self.export_dataframe(df_mois, 'evolution_temporelle.csv')
    ...
    def repartition_geographique(self):
        ...
        self.export_dataframe(df, 'donnees_geographiques.csv')
    ...
    def ventes_par_gamme(self):
        ...
        # Export des données par gamme
        self.export_dataframe(df, 'ventes_par_gamme.csv')
    ...
    def top_produits(self):
        ...
        # Export des données top produits
        self.export_dataframe(df, 'top_produits_donnees.csv')
    ...
    def analyse_stocks(self):
        ...
        # Export des données de stocks
        self.export_dataframe(df, 'analyse_stocks_complete.csv')
        ...
        if not produits_risque.empty:
            ...
            # Export des alertes stock
            self.export_dataframe(produits_risque, 'alertes_stock.csv')
        else:
    ...
    def analyse_clients(self):
        ...
        self.export_dataframe(df, 'segmentation_clients.csv')
    ...
    def generer_rapport_complet(self):
        ...
        print("=" * 60)
        print(f"Dossier export: {self.export_dir}")
        print(f"Dossier visualisation: {self.visualization_dir}")
        print("=" * 60)
    ...
    def generer_rapport_synthese(self):
        ...
        rapport.append("")
        rapport.append("📁 STRUCTURE DES DOSSIERS:")
        rapport.append(f"  • Export: {os.path.abspath(self.export_dir)}")
        rapport.append(f"  • Visualisations: {os.path.abspath(self.visualization_dir)}")
        rapport.append("")
        rapport.append("📊 FICHIERS CSV EXPORTÉS (dossier export):")
        rapport.append("  • evolution_temporelle.csv")
        rapport.append("  • donnees_geographiques.csv")
        rapport.append("  • ventes_par_gamme.csv")
        rapport.append("  • top_produits_donnees.csv")
        rapport.append("  • analyse_stocks_complete.csv")
        rapport.append("  • alertes_stock.csv (si applicable)")
        rapport.append("  • segmentation_clients.csv")
        rapport.append("")
        rapport.append("📈 VISUALISATIONS GÉNÉRÉES (dossier visualization):")
        ...
        rapport.append("")
        rapport.append("📍 EMPLACEMENTS:")
        rapport.append(f"  Export: {os.path.abspath(self.export_dir)}")
        rapport.append(f"  Visualisation: {os.path.abspath(self.visualization_dir)}")
        rapport.append("")
        rapport.append("=" * 70)
        
        # Sauvegarde du rapport dans le dossier export
        rapport_path = f'{self.export_dir}/rapport_synthese.txt'
        with open(rapport_path, 'w', encoding='utf-8') as f:
            f.write('\n'.join(rapport))
        
        print(f"📄 Rapport de synthèse généré: {rapport_path}")
        print(f"📁 Fichiers CSV dans: {os.path.abspath(self.export_dir)}")
        print(f"📁 Visualisations dans: {os.path.abspath(self.visualization_dir)}")
...
def main():
    ...
    while True:
        ...
        if choix == '1':
            ...
            import subprocess
            import platform
            export_path = os.path.abspath(visualizer.export_dir)
            visualization_path = os.path.abspath(visualizer.visualization_dir)
            print(f"\n📁 Dossier export: {export_path}")
            print(f"📁 Dossier visualization: {visualization_path}")
            
            # Ouvrir le dossier parent pour montrer les deux dossiers
